...
public class DistributedSPARQLManager implements DistributableEntityManager {
						...
						if (!sparqlClassProperty.isPeerUid()) {
							...
							if (!sparqlClassProperty.isUri()
								...
								if (sparqlClassProperty.getType().equals("int")) {
									myModel.add(myModel.createLiteralStatement(
											resourceToAdd,
											myModel.createProperty(sparqlClassProperty
													.getNamespaceURL()
													+ sparqlClassProperty
															.getPropertyName()),
											sparqlClassProperty.getValue()));
								} else if (sparqlClassProperty.getType()
										.equals("long")) {
									myModel.add(myModel.createLiteralStatement(
											resourceToAdd,
											myModel.createProperty(sparqlClassProperty
									...
									if (sparqlClassProperty.getValue() == null) {
										...
										value = (String) sparqlClassProperty
												.getValue();
									}
									myModel.add(myModel.createLiteralStatement(
											resourceToAdd,
							...
							if (sparqlClassProperty.isArray()) {
								...
								if (SparqlClassInspector.getClassInspector()
									...
									if (arrayField.get(objectToAdd) != null) {
										...
										// String array
											...
											String[] array = (String[]) arrayField
													.get(objectToAdd);
								...
								// IEntity types
									...
									IEntity[] array = (IEntity[]) arrayField
											.get(objectToAdd);
									if (array != null) {
										int size = Array.getLength(array);

										for (int i = 0; i < size; i++) {
											IEntity object = (IEntity) Array
													.get(array, i);
											...
											Field fieldLinked = object
													.getClass().getField(
															uri_field);
											String uriToLink = (String) fieldLinked
													.get(object);
	...
	public <T extends IEntity> T loadArray(T objectToLoad, String field,
		...
		try {
			...
			throw new DDBSToolkitException(
					"Error during use of the reflection mechanism", e);
		}
	}

	protected <T extends IEntity> ArrayList<T> conversionResultSet(
			...
			if (!hasArray) {
				...
				while (results.hasNext()) {
					...
					for (SparqlClassProperty myProperty : listProperties) {
						...
						if (!myProperty.isArray()) {
							if (!myProperty.isOptional()
									|| (myProperty.isOptional() && myResult
											.getLiteral(myProperty.getName()) != null)) {
				...
				while (results.hasNext()) {
					...
					for (IEntity entity : resultList) {
						...
						if (myResult.getResource(uri) == null
								|| entityUri.equals(myResult.getResource(uri)
										.toString())) {
							hasEntity = true;
									...
									for (int i = 0; i < sizeArray; i++) {
										...
										// Float array
											...
											if (myResult.getLiteral(
													myProperty.getName())
													.getFloat() == Array
													.getFloat(listObject, i)) {
												objectExist = true;
											}
										} else if (property
												.equals("[Ljava.lang.String;")) {
											if (myResult
													.get(myProperty.getName())
													.toString()
													.equals(Array.get(
															listObject, i))) {
												objectExist = true;
							...
							if (myProperty.isArray()
								...
								if (listObject != null) {
									...
									for (int i = 0; i < sizeArray; i++) {
										if (property.equals("[I")) {
											...
											if (stringResult.equals(Array.get(
													listObject, i))) {
												objectExist = true;
											}
										}
								...
								if (!objectExist) {
									if (property.equals("[I")) {
										...
										for (int i = 0; i < sizeArray; i++) {
											Array.set(array, i, Array.getLong(
													listObject, i));
										}
										...
										for (int i = 0; i < sizeArray; i++) {
											Array.set(array, i, Array.getFloat(
													listObject, i));
										}
										;
